...
- `usage::*` (`usage_snapshot`, `usage_refresh`, `usage_connect`, `usage_disconnect`): plan-usage status for Claude and Codex, read from each CLI's own credential store (macOS Keychain or `~/.claude/.credentials.json`). `usage_snapshot` is synchronous and serves the cache; `usage_refresh` / `usage_connect` are `async` + `spawn_blocking` because they shell out to the Keychain and make blocking HTTP calls. `UsageState` keeps the cache lock separate from the fetch lock: the cache lock stays microsecond-short so main-thread reads never block, and the fetch lock serializes refreshes so Claude's single-use refresh token cannot be raced and stranded.
- `power::set_keep_awake`: toggles a system sleep inhibitor while coding agents work (see agents module). Held assertion is released on app exit and watches the Pide pid so a crash cannot leave it stuck.
...
- **sidebar/** - independent resizable panels: Projects on the left, Files / Source Control on the right. Header buttons collapse each panel separately; widths and collapsed states persist under separate local-storage keys. Both use the active theme. The former sidebar-position preference is retired.
- **source-control/** - git status / stage / commit panel and diff workflow. A clean attached GitHub branch with an upstream and no ahead or behind commits exposes Create PR, deriving GitHub's compare URL from the authorized tracking remote and remote branch before opening it with Tauri's opener. Pide does not create or detect pull requests.
- **git-history/** - History starts with the virtualized commit graph and drills into a same-tab inspector with a linear commit list, full message, changed files, and the existing lazy read-only diff renderer. Back preserves graph search and scroll state.
...
- **agents/** - status and notifications for coding-agent CLIs running inside terminal sessions. `store/agentStore.ts` tracks terminal sessions and notifications; `lib/route.ts` suppresses notifications when focused and visible, uses OS notifications when unfocused, and uses an in-app toast when focused but hidden. Two detection layers feed one `pide:agent-signal` stream: Rust-side PTY detection in `pty/agent_detect.rs` consumes recognized OSC 777 hook markers, and a frontend title tracker (`lib/titleStatus.ts` heuristics + `lib/titleTracker.ts` transitions, wired in `AgentNotificationsBridge`) classifies OSC 0/2 terminal titles (braille spinner frames, Claude/Gemini glyphs, boundary-guarded agent-name keywords) so any agent CLI is tracked without hooks or prior registration; identity falls back to the OSC 133;C command line. Explicit 777 markers mark a session `hookDriven` and outrank title heuristics for it. Sessions and notifications carry a `Space/Project` context shown in the bell and OS notifications, and a leaf's notifications clear when its agent exits or the terminal closes (foreground-command falling edge, title removal, or Rust `exited`). `lib/keepAwake.ts` plus Rust `power::set_keep_awake` hold an OS sleep inhibitor (`caffeinate -w` on macOS, `SetThreadExecutionState` on Windows, `systemd-inhibit` on Linux, all self-releasing if Pide dies) while any session is working, opt-in via Settings with a 2 h staleness cap. Optional hooks for Claude Code, Codex, and Gemini CLI, plus a managed Pi extension (`~/.pi/agent/extensions/pide-notifications.ts`), are installed through `agent_enable_hooks` / `agent_hooks_status`.
- **usage/** - plan-usage chips in the status bar for Claude and Codex. `store/usageStore.ts` holds the snapshot list and refreshes every 5 min via `startAutoRefresh` (wired in `App.tsx`); `withStaleness` marks snapshots stale after 15 min client-side. All network work happens in Rust (`usage::*`), never on the main thread. Backoff is server-side in `UsageState`, so a provider that is failing is not re-hit on every tick.
- **command-palette/** - modal command palette (`CommandPalette.tsx`, `commands.ts`) for actions and navigation.
- **spaces/** - owns ordered Space and Project metadata plus version 2 persistence. `ProjectSidebar` keeps the Space/Project picker visible with search, rename, reorder, add/remove, and locate actions. The selected Project root pins explorer, source control, search, and ordinary new terminals. Project activity aggregates detected terminal agents by tab ownership; input requests take priority over errors, working, unread completion, and idle. Collapsed Spaces retain a summary. Agent launch starts idle; explicit working hooks or title activity start the spinner. Viewing or interacting with a terminal acknowledges its completion/error, while input requests remain until resolved. Project selection opens the highest-priority attention target. Status comes from the existing agent store, without separate polling. Unavailable Projects stay cold until located.

...
const PIDE_MARKER: &[u8] = b"notify;Pide;";
...
impl AgentDetector {
    ...
    fn handle_osc777<F: FnMut(Transition)>(&mut self, pt: &[u8], emit: &mut F) {
        if let Some(tail) = pt.strip_prefix(PIDE_MARKER) {
...
mod tests {
    ...
    #[test]
    fn pide_marker_drives_status() {
        let mut d = AgentDetector::new();
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(run(&mut d, &osc("777;notify;Pide;attention")), vec![Transition::Attention]);
        ...
        assert_eq!(run(&mut d, &osc("777;notify;Pide;finished")), vec![Transition::Finished]);
    }

    #[test]
    fn pide_marker_auto_arms_without_preexec() {
        let mut d = AgentDetector::new();
        assert_eq!(
            run(&mut d, &osc("777;notify;Pide;attention")),
    ...
    fn four_field_marker_ignores_unknown_agent() {
        let mut d = AgentDetector::new();
        assert!(run(&mut d, &osc("777;notify;Pide;evil;attention")).is_empty());
        // A known agent in the same chunk still works.
        assert_eq!(
            run(&mut d, &osc("777;notify;Pide;codex;attention")),
    ...
    fn four_field_marker_drives_status_after_preexec() {
        ...
        assert_eq!(run(&mut d, &osc("777;notify;Pide;gemini;attention")), vec![Transition::Attention]);
        assert_eq!(run(&mut d, &osc("777;notify;Pide;gemini;working")), vec![Transition::Working]);
        assert_eq!(run(&mut d, &osc("777;notify;Pide;gemini;finished")), vec![Transition::Finished]);
    }
    ...
    fn error_marker_emits_error_and_sets_waiting() {
        ...
        run(&mut d, &osc("133;C;claude"));
        assert_eq!(
            run(&mut d, &osc("777;notify;Pide;claude;error")),
            vec![Transition::Error]
        );
        // Waiting after error: a later working marker re-emits Working.
        assert_eq!(
            run(&mut d, &osc("777;notify;Pide;claude;working")),
            vec![Transition::Working]
        );
    }
}

...
const FISH_REINSTALL_PROMPT: &str =
    "functions -q __pide_install_prompt; and __pide_install_prompt";
...
pub struct ShellInfo {
    pub name: String,
    pub path: String,
    /// True when Pide injects OSC 7/133 integration for this shell (cwd
...
fn apply_common(cmd: &mut CommandBuilder, cwd: Option<String>, blocks: bool) {
    ...
    cmd.env("PIDE_TERMINAL", "1");
    if blocks {
        cmd.env("PIDE_BLOCKS", "1");
    }
...
mod unix {
    ...
    fn integration_root() -> Result<PathBuf, String> {
        ...
        let root = home.join(".cache").join("pide").join("shell-integration");
    ...
    fn prepare_fish_conf_d() -> Result<(), String> {
        ...
        fs::create_dir_all(&dir).map_err(|e| format!("create {}: {e}", dir.display()))?;
        write_if_changed(&dir.join("pide.fish"), FISH_INIT)?;
        Ok(())
    }
...
mod windows {
        ...
        match (shell_kind, integration) {
            (
                ...
                if let Some(user_zdotdir) = user_zdotdir {
                    args.push(format!("PIDE_USER_ZDOTDIR={user_zdotdir}"));
                }
    ...
    fn prepare_wsl_integration_dir(distro: &str, shell: &str) -> Result<(String, PathBuf), String> {
        ...
        let linux_dir = format!(
            "{}/.cache/pide/shell-integration/{shell}",
    ...
    fn prepare_wsl_fish_conf_d(distro: &str) -> Result<(), String> {
        ...
        let unc_dir = crate::modules::workspace::wsl_path_to_unc(distro, &linux_dir);
        fs::create_dir_all(&unc_dir).map_err(|e| format!("create {}: {e}", unc_dir.display()))?;
        let unc_file = unc_dir.join("pide.fish");
    ...
    mod tests {
        ...
        fn builds_wsl_zsh_launch_spec_with_env_and_login() {
            let spec = build_wsl_launch_spec(
                ...
                ShellKind::Zsh,
                WslShellIntegration::Zsh {
                    zdotdir: "/home/vinicios/.cache/pide/shell-integration/zsh".into(),
                    user_zdotdir: None,
                },
            );
            assert_eq!(
                ...
                vec![
                    ...
                    "--exec".to_string(),
                    "env".to_string(),
                    "ZDOTDIR=/home/vinicios/.cache/pide/shell-integration/zsh".to_string(),
        ...
        fn builds_wsl_zsh_launch_spec_with_user_zdotdir_probe() {
            ...
            assert_eq!(
                ...
                vec![
                    ...
                    "--exec".to_string(),
                    "env".to_string(),
                    "PIDE_USER_ZDOTDIR=/home/vinicios/.config/zsh".to_string(),
                    "ZDOTDIR=/home/vinicios/.cache/pide/shell-integration/zsh".to_string(),
                    "/usr/bin/zsh".to_string(),
                    "-l".to_string(),
                ]
        ...
        fn builds_wsl_bash_launch_spec_with_rcfile() {
            let spec = build_wsl_launch_spec(
                ...
                WslShellIntegration::Bash {
                    rcfile: "/home/vinicios/.cache/pide/shell-integration/bash/bashrc".into(),
                },
            );
            assert_eq!(
                spec.args,
                vec![
                    ...
                    "/bin/bash".to_string(),
                    "--rcfile".to_string(),
                    "/home/vinicios/.cache/pide/shell-integration/bash/bashrc".to_string(),
                    "-i".to_string(),
                ]
            );
        }

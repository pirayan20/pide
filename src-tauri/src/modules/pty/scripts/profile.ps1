# pide-shell-integration (PowerShell)
...
if ($global:__PIDE_HOOKS_LOADED) { return }
$global:__PIDE_HOOKS_LOADED = $true
...
if (Test-Path Function:prompt) {
    Copy-Item Function:prompt Function:__pide_user_prompt -Force -ErrorAction SilentlyContinue
}

function global:__pide_urlencode {
    param([string]$s)
...
function global:__pide_install_readline {
    if ($global:__pide_readline_done) { return }
    if (-not (Test-Path Function:PSConsoleHostReadLine)) { return }
    $global:__pide_readline_done = $true
    ...
    Copy-Item Function:PSConsoleHostReadLine Function:global:__pide_user_readline -Force
    function global:PSConsoleHostReadLine {
        try {
            $line = __pide_user_readline
        } catch {
            # Self-heal: restore the original reader so a broken wrapper can
            # never lock the shell out of input.
            Copy-Item Function:__pide_user_readline Function:global:PSConsoleHostReadLine -Force
            return ''
        }
        try {
            if ($line -is [string] -and $line.Trim().Length -gt 0) {
                $global:__pide_block_seen = $true
                $esc = [char]27
...
function global:prompt {
    ...
    if ($env:PIDE_BLOCKS) {
        ...
        $gap = if ($global:__pide_block_seen) { "`n`n" } else { "`n" }
        return "$oscD$oscA$osc7$gap$oscB"
    }

    $original = if (Test-Path Function:__pide_user_prompt) {
        try { & __pide_user_prompt } catch { "PS $((Get-Location).Path)> " }
    } else {

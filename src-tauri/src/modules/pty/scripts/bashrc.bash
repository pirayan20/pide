...
if [ -z "$__PIDE_HOOKS_LOADED" ]; then
  ...
  _pide_precmd() {
    ...
    printf '\e]7;file://%s%s\e\\' "${HOSTNAME:-$(uname -n 2>/dev/null)}" "$(_pide_urlencode "$PWD")"
    ...
    elif [ -z "$__PIDE_PS1_INJECTED" ]; then
      PS1='\[\e]133;B\e\\\]'"$PS1"
      __PIDE_PS1_INJECTED=1
    fi
    printf '\e]133;A\e\\'
  }
  ...
  case ":${PROMPT_COMMAND:-}:" in
    *":_pide_precmd:"*) ;;
    *) PROMPT_COMMAND="_pide_precmd${PROMPT_COMMAND:+;$PROMPT_COMMAND}" ;;
  esac
  ...
  if [ "${BASH_VERSINFO[0]:-0}" -gt 4 ] \
    ...
    if [ -n "$PIDE_BLOCKS" ]; then
      ...
      PS0='\[\e]133;C\e\\\]${_pide_noop[$((_pide_block_seen=1))]}'"${PS0:-}"
    else
      PS0='\[\e]133;C\e\\\]'"${PS0:-}"
    fi
  fi

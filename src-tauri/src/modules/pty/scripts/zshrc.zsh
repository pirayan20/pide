...
# zsh, so we shadow $? into `_pide_ret`.

{
  _pide_user_zdotdir="${PIDE_USER_ZDOTDIR:-$HOME}"
  [ -f "$_pide_user_zdotdir/.zshrc" ] && source "$_pide_user_zdotdir/.zshrc"
...
if [[ -z "$__PIDE_HOOKS_LOADED" ]]; then
  __PIDE_HOOKS_LOADED=1
  ...
  _pide_precmd() {
    local _pide_ret=$?
    printf '\e]133;D;%s\e\\' "$_pide_ret"
    printf '\e]7;file://%s%s\e\\' "${HOST}" "$(_pide_urlencode "$PWD")"
  ...
  _pide_preexec() {
    ...
    [[ -n "$PIDE_BLOCKS" ]] && _pide_block_seen=1
    local cmd="${1//[[:cntrl:]]/ }"
    printf '\e]133;C;%s\e\\' "${cmd[1,256]}"
  }

  if (( $+functions[add-zsh-hook] )); then
    add-zsh-hook precmd _pide_precmd
    add-zsh-hook preexec _pide_preexec
  fi

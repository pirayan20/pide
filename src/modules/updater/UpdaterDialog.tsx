...
function distroCommand(key: DistroKey, version: string): string {
  switch (key) {
    case "arch":
      return "yay -S pide-bin";
    case "debian":
      return `sudo apt install ./Pide_${version}_amd64.deb`;
    case "fedora":
      return `sudo dnf install ./Pide-${version}-1.x86_64.rpm`;
  }
}
          ...
          <DialogTitle>
            {ready
              ...
              : downloading
                ? "Downloading update…"
                : manual
                  ? `Pide v${manual.version} is available`
                  : `Pide v${update?.version} is available`}
          </DialogTitle>
          <DialogDescription>
            {ready
              ? "Restart Pide to finish installing."
              : downloading
                ? progress !== null
